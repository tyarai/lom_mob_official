...
class Sighting {
  ...
  void saveToDatabase() async {
    ...
    _uid.then((_currentUID) async {
      ...
      this.date = this.date == null ? DateTime
          .now()
          .millisecondsSinceEpoch
          .toDouble() : this.date;
      this.created = this.created == null ? DateTime
          .now()
          .millisecondsSinceEpoch
          .toDouble() : this.created;
      ...
      this.photoFileName = this.photoFileName != null ? this.photoFileName : defaultImage.photoAssetPath(ext:Constants.imageType);
  ...
  /*
  static Widget buildSightingPhoto(Sighting sighting,
      {double width = Constants.listViewImageWidth, double height = Constants
          .listViewImageHeight}) {
    Widget widget;
  ...
  static Widget getImage(Sighting sighting) {
    ...
    if (sighting != null && sighting.photoFileName.length != 0){
      ...
      if(! sighting.photoFileName.startsWith(Constants.appImagesAssetsFolder)) {
        ...
        if (file.existsSync()) {
          return Container(child: Image.file(file)); // Return image from Documents
        }
      }else{
        return Container(child: Image.asset(sighting.photoFileName)); // Return image from assets
      }
    }

    return Container(
      child:Icon(Icons.camera,size:Constants.cameraPhotoPlaceHolder,color:Constants.cameraPlaceHolderColor),
    );

  }

  static Widget buildCellInfo(Sighting sighting,{bool lookInAssetsFolder = false,CrossAxisAlignment crossAlignment = CrossAxisAlignment.start}) {


    String formattedDate = DateFormat.yMMMMd("en_US").format(DateTime.fromMillisecondsSinceEpoch(sighting.date.toInt()));
    ...
    return Column(
          ...
          children: <Widget>[
            ...
            Container(height: 5),
            Text(sighting.speciesName,style: Constants.sightingSpeciesNameTextStyle,),
            ...
            Row(
                ...
                children:<Widget>[
                  ...
                  Expanded(
                    ...
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children:<Widget>[ Text(sighting.speciesCount.toString() + " species observed",style: Constants.sightingSpeciesCountTextStyle,)]),
                  ),
                  Container(width: 10),
                  Expanded(
                    child: Column(
                        crossAxisAlignment: CrossAxisAlignment.end,
                        children:<Widget>[ Text(formattedDate,style: Constants.sightingSpeciesCountTextStyle,)]),
                  ),
            ]),
            Container(height: 5),
            Text(sighting.placeName,style: Constants.sightingSpeciesCountTextStyle,),
            Container(height: 10),
            Text(sighting.title,style: Constants.sightingTitleTextStyle,)
